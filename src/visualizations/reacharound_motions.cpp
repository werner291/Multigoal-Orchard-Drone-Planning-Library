...
REGISTER_VISUALIZATION(retreat_motions) {
	...
	// We may then combine and cross-link these paths to create something that reaches around the fruit in several places.
	...
	RobotState end_state = generateRandomRobotState(robot, rng);
	...
	path = subdivided(path, 20);

	// We'll visualize it with a ladder trace:
