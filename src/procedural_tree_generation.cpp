...
void make_tree_branches(const Eigen::Isometry3d &root_at, unsigned int branching_depth, double root_radius,
    ...
    if (branching_depth > 0) {
        ...
        for (int i = 0; i < split_number; i++) {
            ...
            // Pre-transform?
            local_root *= Eigen::AngleAxisd(distr(eng), Eigen::Vector3d::UnitX()) *
                          Eigen::AngleAxisd(distr(eng), Eigen::Vector3d::UnitY());
...
std::vector<Apple> spawn_apples(const std::vector<DetachedTreeNode> &flattened,
    ...
    std::uniform_int_distribution<size_t> tree_branch_selection(1 /* No apples on the main trunk */,
                                                                flattened.size() - 1);
    ...
    while (apples.size() < NUMBER_OF_APPLES) {
        ...
        auto apple_xform = frame_on_branch(distr(eng) * (float) M_PI * 2.0f, distr(eng), node) *
                           Eigen::Translation3d(Eigen::Vector3d(0, 0, apple_radius));
