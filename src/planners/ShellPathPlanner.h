...
class ShellPathPlanner : public MultiGoalPlanner {
	...
	typedef const std::function<MoveItAppleSphereShell(const AppleTreePlanningScene& scene_info)> MakeShellFn;
...
public:
	...
	[[nodiscard]] PlanResult assembleFullPath(
            const ompl::base::SpaceInformationPtr &si,
            const std::vector<ompl::base::GoalPtr> &goals,
            const MoveItAppleSphereShell &ompl_shell,
	...
	[[nodiscard]] ompl::geometric::PathGeometric retreat_move_probe(
            const std::vector<ompl::base::GoalPtr> &goals,
            const MoveItAppleSphereShell &ompl_shell,
            PlanResult &result,
	...
	[[nodiscard]] std::optional<ompl::geometric::PathGeometric> planFirstApproach(
            const ompl::base::State *start,
            ompl::geometric::PathGeometric &approach_path);

	[[nodiscard]] std::vector<size_t> computeApproachOrdering(
            const ompl::base::State *start,
            const std::vector<ompl::base::GoalPtr> &goals,
            const std::vector<std::pair<size_t, ompl::geometric::PathGeometric>> &approaches,
            const MoveItAppleSphereShell& distance_heuristics) const;

	[[nodiscard]] std::vector<std::pair<size_t, ompl::geometric::PathGeometric>>
	planApproaches(const ompl::base::SpaceInformationPtr &si,
				   const std::vector<ompl::base::GoalPtr> &goals,
				   const MoveItAppleSphereShell &ompl_shell,
				   ompl::base::PlannerTerminationCondition &ptc) const;

    [[nodiscard]] std::optional<ompl::geometric::PathGeometric> planApproachForGoal(
            const ompl::base::SpaceInformationPtr &si,
            const MoveItAppleSphereShell &ompl_shell,
