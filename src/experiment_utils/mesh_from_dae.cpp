...
mgodpl::Mesh mgodpl::from_dae(const std::string &dae_file) {
	...
	const aiScene *scene = importer.ReadFile(dae_file,
	...
	// This is a tree mesh if it contains the word "fruit", "apples", "leaves", or "trunk"
	bool is_tree_mesh = dae_file.find("apples") != std::string::npos ||
	                    dae_file.find("fruit") != std::string::npos ||
		...
		for (unsigned int j = 0; j < ai_mesh->mNumVertices; ++j) {
			...
			if (is_tree_mesh) {
				mesh.vertices.emplace_back(
					vertex.x / (100.0 / CENTIMETERS_PER_INCH),
					// There appears to me some sort of cm/mm conversion going on with DAE, hence the weird division
					vertex.z / (100.0 / CENTIMETERS_PER_INCH),
					// Z and Y are swapped intentionally
