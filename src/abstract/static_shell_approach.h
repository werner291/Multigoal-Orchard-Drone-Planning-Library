...
template<
		...
		typename ShellSpace,
		typename Path,
		typename ApproachPathPlanner = std::function<std::optional<Path>(const State&, const Goal&, const ShellSpace&)>,
		typename TSPPlanner = std::function<std::vector<int>(const std::vector<Path>&, const std::vector<bool>&)>,
		typename PathOptimizer = std::function<Path(const Path&)>
>
Path plan_path_static_goalset(const State& initial_state,
	...
	for (size_t i = 0; i < visit_order.size() - 1; i++) {
		...
		Path gtg = assemble_goal_to_goal_path(
				approach_paths[visit_order[i]], shell_space, approach_paths[visit_order[i + 1]]
				);
