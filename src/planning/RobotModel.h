...
namespace mgodpl::robot_model {
	 ...
	 *
	 * Note: there is no absolute frame of reference; the model is defined in terms of relative transformations between
	 * links and joints.
	 */
	class RobotModel {
	...
	public:
		...
		struct Joint {
			...
			/// The transformations from the link's origin to the joint's frame of reference. (Note that you may have to
			/// inverse these in some cases.)
		 ...
		 * @param joint_values 	The values of the joint variables; make sure there are enough values (consult
		 * n_variables()).
		...
		[[nodiscard]] static inline size_t n_variables(const JointTypeSpecific &variablePart) {
			switch (variablePart.index()) {
				case 0:
					return 1; // Revolute joint
				case 1:
					return 0; // Fixed joint
				default:
		...
		/**
		 * @brief Finds a link by name and returns its ID. (Linear search; you might want to index links by name if you
		 * use this a lot.)
		...
		/**
		 * @brief Finds a joint by name and returns its ID. (Linear search; you might want to index joints by name if
		 * you use this a lot.)
	...
	/**
	 * @brief Computes the forward kinematics of a robot model, given a set of joint values and a link from which to
	 ...
	 * @param from_link_transform	The transform of the root_link relative to the root link (if identity, the computed
	 *								transforms will be relative to the root_link).
	 * @return 						The forward kinematics result.
	 */
	ForwardKinematicsResult
	forwardKinematics(const RobotModel &model,
					  const std::vector<double> &joint_values,
					  const RobotModel::LinkId &root_link,
					  const math::Transformd &from_link_transform = math::Transformd::identity());

	/**
	 * A variant of ForwardKinematics that takes a RobotState instead of a vector of joint values.
	 */
	ForwardKinematicsResult
	forwardKinematics(const RobotModel &model, const RobotState &state, const RobotModel::LinkId &root_link = 0);
