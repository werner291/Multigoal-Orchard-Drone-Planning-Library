...
namespace mgodpl
{
    ...
    // Step 3:
    // Break those down further into range start/end events and sort by longitude.
    enum StartEnd { Start, End };

    struct EdgePairEvent
    {
        StartEnd start_end;
        OccupiedLatitudeRange& range;
    ...
    {
        // Step 2: Break triangles down into edge pairs:
        std::vector<OccupiedLatitudeRange> edge_pairs;
        edge_pairs.reserve(triangles.size() * 2);
        for (const auto& triangle : triangles)
        {
            auto vertices = sorted_relative_vertices(triangle, center);

            OccupiedLatitudeRange e1 {
                Edge{vertices[0].local_vertex, vertices[1].local_vertex},
                Edge{vertices[0].local_vertex, vertices[2].local_vertex},
                .min_longitude = vertices[0].longitude,
                .max_longitude = vertices[1].longitude
            };

            OccupiedLatitudeRange e2 {
                Edge{vertices[1].local_vertex, vertices[2].local_vertex},
                Edge{vertices[0].local_vertex, vertices[2].local_vertex},
                .min_longitude = vertices[1].longitude,
                .max_longitude = vertices[2].longitude
            };

            // Check if the vertex is above or below:
            if (vertex_is_above_long_edge(vertices))
            {
                std::swap(e1.min_latitude_edge, e1.max_latitude_edge);
                std::swap(e2.min_latitude_edge, e2.max_latitude_edge);
            }

            edge_pairs.push_back(e1);
            edge_pairs.push_back(e2);
        }
        ...
        std::vector<EdgePairEvent> events;
        for (auto& edge_pair : edge_pairs)
        {
            events.push_back(EdgePairEvent{.start_end = Start, .range = edge_pair});
            events.push_back(EdgePairEvent{.start_end = End, .range = edge_pair});
        }
        ...
        struct StackFrame
        {
            std::vector<EdgePairEvent>::iterator begin;
            std::vector<EdgePairEvent>::iterator end;
        };
    }
    ...
    {
        double longitude;
        std::vector<FreeLatitudeRange> free_ranges;
    };

    void recurse(
        const Longitude& min_longitude,
        const Longitude& max_longitude,
        const std::vector<EdgePairEvent>::iterator events_in_range_begin,
        const std::vector<EdgePairEvent>::iterator events_in_range_end)
    {
        if (events_in_range_begin == events_in_range_end)
        {
            // Treat base case.
        } else
        {
            ...
            // Median iterator:
            auto median = events_in_range_begin + std::distance(events_in_range_begin, events_in_range_end) / 2;

            for (auto scan_itr = events_in_range_begin; scan_itr != events_in_range_end; ++scan_itr)
            {
