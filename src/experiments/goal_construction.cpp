...
int main() {
	...
	std::cout << "Collision probability computation took "
			  << std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count() << " ms."
			  << std::endl;
	std::cout << "That is, " << std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count() /
								(double) targets.size() << " us per target." << std::endl;
				...
				if (cell.triangles.empty()) {
					...
					math::Transformd flying_base_tf{
							...
							.orientation = math::Quaterniond::fromAxisAngle(math::Vec3d::UnitZ(),
																			spherical_geometry::longitude(vec) +
																			M_PI / 2.0)
					};

					const auto &fk = mgodpl::robot_model::forwardKinematics(robot,
																			arm_angles,
																			flying_base,
																			flying_base_tf);
					...
					bool collision = check_link_collision(robot.getLinks()[stick],
														  tree_trunk_object,
														  fk2.forLink(stick));
