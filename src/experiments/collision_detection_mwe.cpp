	...
	for (const double radius: {0.5, 1.0, 2.0, 5.0, 10.0}) {
		for (const int n_triangles: {10, 100, 1000, 10000, 100000}) {
			...
			for (size_t i = 0; i < n_triangles; ++i) {
				fcl::Vector3d a(rng.uniformReal(radius, radius),
								rng.uniformReal(radius, radius),
								rng.uniformReal(radius, radius));
				fcl::Vector3d b(rng.uniformReal(radius, radius),
								rng.uniformReal(radius, radius),
								rng.uniformReal(radius, radius));
				fcl::Vector3d c(rng.uniformReal(radius, radius),
								rng.uniformReal(radius, radius),
								rng.uniformReal(radius, radius));
			...
			for (size_t sample_i = 0; sample_i < N_SAMPLES; ++sample_i) {
				...
				fcl::Transform3d tf = fcl::Translation3d(rng.uniformReal(-radius, radius),
														 rng.uniformReal(-radius, radius),
														 rng.uniformReal(-radius, radius)) *
									  fcl::Quaterniond(rng.uniformReal(-1.0, 1.0),
													   rng.uniformReal(-1.0, 1.0),
													   rng.uniformReal(-1.0, 1.0),
													   rng.uniformReal(-1.0, 1.0)).normalized();
